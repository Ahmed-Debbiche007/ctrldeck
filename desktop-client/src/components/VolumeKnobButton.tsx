...
interface VolumeKnobButtonProps {
  value: number;
...
export function VolumeKnobButton({ value, onChange, color, name }: VolumeKnobButtonProps) {
  ...
  const calculateValueFromPosition = useCallback((clientX: number, clientY: number) => {
    ...
    // atan2 returns angle in radians, where 0 is pointing right (3 o'clock)
    const angleRad = Math.atan2(clientY - centerY, clientX - centerX);
    let angleDeg = angleRad * (180 / Math.PI);

    // Convert to our coordinate system:
    // - 0% should be at bottom-left (7 o'clock position, which is 135 degrees from top)
    // - 100% should be at bottom-right (5 o'clock position, which is 45 degrees from top)
    // Standard atan2: 0° = right, 90° = down, -90° = up, 180/-180 = left
    ...
    // Our knob range: 
    // - Start (0%): 225° (7 o'clock, bottom-left)
    // - End (100%): 315° (5 o'clock, bottom-right)
    // This gives us a 270° sweep going clockwise from 225° to 315° (through 0°/360°)
    
    // Map the angle to a value
    let normalizedAngle;
    if (angleDeg >= 225) {
      // From 225° to 360° -> 0% to 50%
      normalizedAngle = angleDeg - 225;
    } else if (angleDeg <= 315 - 270) {
      // From 0° to 45° (315-270=45) -> continues from where we left off
      normalizedAngle = angleDeg + 135; // 0° becomes 135°, 45° becomes 180°
    } else {
      // Dead zone (bottom area between 45° and 225°)
      // Snap to nearest extreme
      if (angleDeg < 135) {
        normalizedAngle = 270; // Snap to 100%
      } else {
        normalizedAngle = 0; // Snap to 0%
      }
    }
    
    // Convert to percentage (270° range = 100%)
