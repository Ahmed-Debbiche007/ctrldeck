...
export function IconPicker({ selectedIcon, onSelect, color = '#3b82f6' }: IconPickerProps) {
  return (
    <div className="grid grid-cols-6 xs:grid-cols-7 sm:grid-cols-8 gap-1.5 sm:gap-2 max-h-48 sm:max-h-64 overflow-y-auto p-1.5 sm:p-2 bg-gray-800/50 rounded-lg">
      {commonIcons.map((iconName) => {
        ...
        return (
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-lg transition-all touch-target ${
              isSelected
                ? 'bg-blue-500/30 ring-2 ring-blue-500'
                : 'hover:bg-gray-700/50 active:bg-gray-700/50'
            }`}
            title={iconName}
          >
            <IconComponent
